...
class StructureFactor:
    ...
    def read_data_and_analyze(
        ...
        sf_tuple = read_and_average_SF(
            *output_files,
            self.options.anisotropic,
            self.options.slab_truncation,
        )
...
def parse_command_line_arguments(
    ...
    parser.add_argument('-slab', '--slab-truncation', action='store_true',
                        default=False, dest='slab_truncation', help='When '
                        'using slab truncation there can be non-finite values '
                        'in the S(G) terms which correspond to zeroes in the '
                        'Coulomb kernel. These are zeroed when this flag is '
                        'set. WARNING: this overrides the safe behavior '
                        'where the script exits when encountering non-finite '
                        'values. It is up to the user to check the '
                        'correlation energy from summing S(G)V(G) is correct '
                        'and the non-finite values are the result of the '
                        'truncation scheme behaving appropriately.')
...
def read_Sg(S_G_file: str, slab_truncation: bool = False) -> Array:
    ...
    slab_truncation : bool, default=False
        If true, the slab truncation is was used in the generation
        of the SF.elements file, and there are likely non-finite values
        which are zeroed to prevent analysis failure down stream.
    ...
    if slab_truncation:
        S_G[np.logical_not(np.isfinite(S_G))] = 0.0
...
def read_and_average_SF(
    ...
    anisotropic : bool, default=False
        ...
        is done with or with G vector averaging/matching.
    slab_truncation : bool, default=False
        Indicates if the slab truncation was used in the
        MP2/CCSD calculation that generated the S(G) data.
        If the slab truncation is used there are non-finite values
        which are zeroed to prevent analysis failure.
    ...
    if slab_truncation:
        warn('\n"-slab" flag encountered, therefore overriding non-finite '
             'terms in SF.elements, it is up to the user to ensure this is a '
             'reasonable thing to do!', stacklevel=2)
    ...
    for files in zip(Gvector_files, Coulomb_files, S_G_files):
        ...
        if anisotropic:
            ...
            SFi['S_G'] = read_Sg(files[2], slab_truncation)
